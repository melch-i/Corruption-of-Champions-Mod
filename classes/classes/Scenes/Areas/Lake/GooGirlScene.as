...
//New Perk – Slime Core (requires goo player, random drop rate?)
		...
		{
			if (rand(4) == 0 && player.hasStatusEffect(StatusEffects.SlimeCraving) && !player.hasPerk(PerkLib.SlimeCore) && player.isGoo() && player.gooScore() >= 4) {
...
//Defeat – Female
		private function femaleLoseToGooGal():void
		{
			...
			if (player.pregnancyIncubation <= 280 && player.pregnancyIncubation >= 1) {
				...
				outputText("  She gives you a fond bop on the chin and a loving stroke on your belly before sloshing back into the lake, her seething heat still clinging to your body like a lingering hug.");
			}
...
//VICTORY
		public function beatUpGoo():void
		{
			...
			if (player.gender == 0 || player.lust < 33 || flags[kFLAGS.SFW_MODE] > 0) {
				outputText("The goo-girl, while an unusual creature, seems unable to communicate and clearly has nothing of value.  Of no particular use in your quest, you shoo the dripping blob back to the shore. She seems disappointed at first, but bounces back quickly enough, spotting movement in the lake.  She splashes in and takes off at top speed, " + gooColor9() + " blur while you take your leave.");
				cleanupAfterCombat();
			}
			else {
